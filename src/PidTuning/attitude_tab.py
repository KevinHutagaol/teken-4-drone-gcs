...
class AttitudeTab(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.Kp_Att = self.default_Kp
        self.Ki_Att = self.default_Ki
        self.Kd_Att = self.default_Kd
        self.tuning_mode = "Roll"
        self.init_ui()
        self.init_graphAtt_plot()
        self.update_graphAtt_plot()  # Tampilkan chart awal
    ...
    def update_graphAtt_plot(self):
        ...
        w = self.graphAtt.width() / self.graphAtt_canvas.figure.dpi
        h = self.graphAtt.height() / self.graphAtt_canvas.figure.dpi
        self.graphAtt_fig.set_size_inches(max(w, 2), max(h, 1), forward=True)
        ...
        bars = self.graphAtt_ax.bar(x_labels, y_values, color=bar_colors, width=0.6)
        self.graphAtt_ax.set_xlabel('Komponen')
        self.graphAtt_ax.set_ylabel('Nilai')
        self.graphAtt_ax.set_title(f"PID - {self.tuning_mode} Attitude")
        self.graphAtt_ax.grid(True, linestyle='--', alpha=0.5)

        # Tambahkan value di atas bar
        for bar in bars:
            height = bar.get_height()
            self.graphAtt_ax.text(
        ...
        self.graphAtt_fig.tight_layout()

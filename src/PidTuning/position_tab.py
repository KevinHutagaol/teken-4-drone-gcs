...
class PositionTab(QtWidgets.QWidget):
    ...
    def pause_control(self):
        print(f"Position control paused for {self.tuning_mode}")
        
    def continue_control(self):
        print(f"Position control continued for {self.tuning_mode}")
    ...
    def submit_pid_values(self):
        ...
        print(f"Submitting Position PID values for {self.tuning_mode}: P={p}, I={i}, D={d}")
        self.pid_submitted.emit(self.tuning_mode, p, i, d)
        
    def load_pid_from_drone(self):
        """Request to load PID values from drone"""
        print(f"Loading Position PID values from drone for {self.tuning_mode}")
